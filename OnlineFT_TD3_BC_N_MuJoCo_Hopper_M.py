...
# Initialise and load pre-trained agent
...
agent.actor_target.load_state_dict(torch.load("YourPathHere", map_location=device))
...
while env_steps < max_steps + 1:
    ...
    while not done:
        ...
        noise = np.random.normal(0, action_noise, size=action_dim)
