...
class Actor(nn.Module):
    ...
    def sample_normal(self, state):
        ...
        mean_action = self.max_action * torch.tanh(mu)

        return actions, log_prob_action, mean_action
...
class Agent():
    ...
    def train_offline(self, replay_buffer, iterations=1, beta=0.0, dep_targets=True):
        ...
        for it in range(iterations):
            ...
            # Critic loss #
            with torch.no_grad():
                next_action, log_next_action, _ = self.actor.sample_normal(next_state)
            ...
            actions, log_actions, mean_action = self.actor.sample_normal(state)
            critic_value = self.critic(state, actions).min(0)[0]
            critic_value -= (alpha * log_actions)
            critic_value /= critic_value.abs().mean().detach()
            actor_loss = -critic_value.mean() + beta * F.mse_loss(action, mean_action)
